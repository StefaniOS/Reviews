...
class ReviewCell: UITableViewCell {
    ...
    func configure(with viewModel: ReviewViewModel?) {
        if let viewModel = viewModel {
            ...
            highlightStars(index: viewModel.rating)
        }
        
        updateAppearance(viewModel)
    }
    ...
    func updateAppearance(_ viewModel: ReviewViewModel?) {
        
        if let _ = viewModel {
            avatarLabel.backgroundColor = UIColor.Custom.blue
            titleLabel.backgroundColor = .white
            subtitleLabel.backgroundColor = .white
            stopShimmerAnimation(for: [titleLabel, subtitleLabel])
        } else {
            avatarLabel.backgroundColor = .lightGray
            titleLabel.backgroundColor = .lightGray
            subtitleLabel.backgroundColor = .lightGray
            highlightStars(index: 0)
            
            avatarLabel.text = ""
            titleLabel.text = ""
            subtitleLabel.text = ""
            textView.text = ""
            startShimmerAnimation(for: [titleLabel, subtitleLabel])
        }
    }
}
